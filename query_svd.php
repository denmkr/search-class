...
class Matrix {
    ...
    public function SVD($matrix){
        ...
        $matrices['W'] = $W;
        $matrices['V'] = $V;
        $matrices['Vt'] = $this->matrixTranspose($V);
        $matrices['Rank'] = $rank;
        $matrices['K'] = $k;
...
/* Reading file and getting documents text */
$articlesFile = json_decode(file_get_contents('myfile.json'), true);
$articles = $articlesFile['articles'];

$documents = array();
foreach ($articles as $key => $article) {
	array_push($documents, $article['title']['mystem'] . " " . $article['abstract']['mystem']);
}
...
$A = array();
foreach ($mergedTerms as $key => $term) {
	$strings = array();
	foreach ($articles as $number => $article) {
		if (in_array($number+1, $term['index'])) {
			$numberOfMatchesInDocuments = substr_count($documents[$number], " " . $term['name'] . " ");
			array_push($strings, $numberOfMatchesInDocuments);
		}
		else array_push($strings, 0);
	}
	array_push($A, $strings);
}
...
$q = array();
foreach ($mergedTerms as $key => $term) {
	if (in_array($term['name'], $queryWords)) array_push($q, 1);
	else array_push($q, 0);
}


/************ 2. DECOMPOSING MATRIX A (USV^T) ************/

$matrixClass = new Matrix;
$USV = $matrixClass->svd($A);
...
/************ 3. RANK APPROXIMATION MATRIX A (USV^T) ************/

/* Set rank 3 */
$Uk = $matrixClass->matrixConstruct($USV['U'], count($USV['U']), $USV['K']);
$Sk = $matrixClass->matrixConstruct($USV['S'], count($USV['S']), $USV['K']);
$Sk = array_slice($Sk, 0, $USV['K']);

$Vk = $matrixClass->matrixConstruct($USV['V'], count($USV['V']), $USV['K']);
$Vtk = $matrixClass->matrixConstruct($USV['Vt'], $USV['K'], $USV['K']);


/************ 4. FINDING DOCUMENT VECTOR ************/

$documentVectors = $Vk;


/************ 5. FINDING NEW QUERY VECTOR ************/
...
for($i=0;$i<count($Sk);$i++) {
	for ($j=0; $j<count($Sk[$i]); $j++) { 
		if ($Sk[$i][$j] == 0) $SkMinus[$i][$j] = 0;
		else $SkMinus[$i][$j] = 1/$Sk[$i][$j];
	}
}

$matrix = new Matrix;
$vector = $matrix->matrixMultiplication($qt, $matrix->matrixMultiplication($Uk, $SkMinus));


/************ 6. FINDING SIM FOR EACH DOCUMENT ************/

$sims = array();

$sumVector = 0;
for ($i=0;$i<$USV['K']; $i++) { 
	$sumVector = $sumVector + pow(floatval($vector[0][$i]), 2);
}
$modulusVector = sqrt($sumVector);

foreach ($documentVectors as $key => $documentVector) {
	$scalarProduct = 0;
	for ($i=0;$i<$USV['K']; $i++) { 
		$scalarProduct = $scalarProduct + floatval($vector[0][$i])*floatval($documentVector[$i]);
	}

	$sumDocuments = 0;
	for ($i=0;$i<$USV['K']; $i++) { 
		$sumDocuments = $sumDocuments + pow(floatval($documentVector[$i]), 2);
	}
	$modulusDocuments = sqrt($sumDocuments);

	$vectorProduct = $modulusVector*$modulusDocuments;
	array_push($sims, $scalarProduct/$vectorProduct);
}

arsort($sims);
echo "DESC SORT: \n";
foreach ($sims as $key => $sim) {
	echo "Document " . $key . " SIM: " . $sim . "\n";
}
